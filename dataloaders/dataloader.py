...
def basicImageDataloader(train_path_list, test_path_list, crop_size=128, train_batch=1, test_batch=1, **kwargs):

    batch_sizes = {'train': train_batch, 'test_batch':test_batch}
    ...
    dataloaders = {x: torch.utils.data.DataLoader(
                                                  image_datasets[x],
                                                  batch_size=batch_sizes[x], shuffle=(x == 'train'),
                                                  **kwargs,
                                                  ) for x in ['train', 'test']}
